...
describe("profile password impersonation route", () => {
  ...
  it("accepts coexisting valid cookies only for an admin tenant target, revokes tenant sessions, and preserves admin cookies", async () => {
    ...
    const middleware = createAuthenticationMiddleware({ allowedOrigins: ["https://app.example.test"], resolveAccessSession: async ({ sessionId }) => sessions[sessionId], getUser: async () => ({ id: "user-a", accountId: "account-a", email: "u@example.test", name: "User", role: "admin", permissions: {} }) });
    const changePassword = vi.fn(async () => ({ id: "user-a", accountId: "account-a", password: "new", name: "User" }));
    const app = express(); app.use(express.json());
    registerProfileRoute(app, { authenticateToken: middleware.authenticateUser, getUser: async () => ({ id: "user-a", accountId: "account-a", password: "old" }), updateUser: vi.fn(), changePassword, hasActiveGlobalAdminCookie: async (request) => Boolean((request.headers.cookie ?? "").includes("politicall_admin_access=")) });
