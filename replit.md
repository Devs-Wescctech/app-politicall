...
- **AI-Powered Competitive Electoral Analysis:** Statistics module using existing user profile data from Settings page (users table) for AI-driven comparative analysis. No separate profile form required - analysis uses data already configured (name, politicalPosition, state, city, lastElectionVotes). POST /api/statistics/analyze generates detailed analysis using OpenAI GPT-4o comparing politician with historical electoral data patterns. Analysis includes: comparison with elected candidates (similarities, differences, success factors), SWOT analysis (strengths, weaknesses, opportunities, threats), strategic recommendations (5 categories: strategic, campaign, communication, alliances, budget), win probability factors (favorable/unfavorable factors + estimated viability score 0-100). Results displayed in professional UI with 5 sections: electoral viability score, elected candidates comparison, SWOT grid, categorized recommendations, probability factors. Analysis saved to statistics_analyses table (id, userId, analysisData JSONB, createdAt). GET /api/statistics/analysis/latest retrieves most recent analysis. Simplified schema removes candidateProfiles table entirely.
