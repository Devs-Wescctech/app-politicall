...
- **AI-Powered Competitive Electoral Analysis:** Statistics module with candidate profile management and AI-driven comparative analysis. Users fill comprehensive profile form (candidate info, ideology, budget, values, proposals, alliances, issues, voter profile, strengths, weaknesses, experience, achievements, recognition). POST /api/statistics/analyze generates detailed analysis using OpenAI GPT-4o comparing candidate with historical electoral data patterns. Analysis includes: comparison with elected candidates (similarities, differences, success factors), SWOT analysis (strengths, weaknesses, opportunities, threats), strategic recommendations (5 categories: strategic, campaign, communication, alliances, budget), win probability factors (favorable/unfavorable factors + estimated viability score 0-100). Results displayed in professional UI with 5 sections: electoral viability score, elected candidates comparison, SWOT grid, categorized recommendations, probability factors. Analysis saved to statistics_analyses table with JSON structure. GET /api/statistics/analysis/latest retrieves most recent analysis.
